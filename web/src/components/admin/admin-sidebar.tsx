...
// MACETE CORRIGIDO: Deteta se a rota atual é filha exata da rota do menu
        ...
        <div className={cx("flex-1 overflow-y-auto px-3 py-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden", collapsed ? "lg:px-2" : "")}>
          <nav className="space-y-4">
            {groups.map((group) => {
              ...
              // MACETE CORRIGIDO: Mantém o grupo aberto se estivermos numa sub-rota que pertença a ele (como /dashboard/docs/editor)
              const activeGroup = group.items.some((item) => isItemActive(pathname, item.href)) || 
                                  (group.title === "Documentação & Engenharia" && pathname.includes("/dashboard/docs"));
